...
logging.basicConfig(handlers=[logging.FileHandler(' Modelling_SVSM(5_cross_validation_and_Tf-Idf)_log.txt','w', 'utf-8')], level=logging.INFO,
...
if __name__ == "__main__":
    ...
    for number_of_features in [100,500,1000,5000,8000]:
        logging.info("-----------------------------TRAINING SVM MODELS WITH {} FEATURES -----------------------------------\n".format(
            number_of_features))
...
#------------------------------------------------------------------Logistic Reagression----------------------------------------------------------------------------------------------
        ...
        lr_model = lr.fit(train_input_bow, np.ravel(train_labels))

        logging.info(lr_model)
        y_pred_lr = lr.predict(test_input_bow)
        ...
        y_pred_prob_lr = lr.predict_proba(test_input_bow)[:, 1]
        logging.info(metrics.roc_auc_score(test_labels, y_pred_prob_lr))
        pickle.dump(lr, open("The_Best_Logistic_Regression_model.pkl", 'wb'))
...
#----------------------------------------------------------------------Naive Bayes-----------------------------------------------------------------------------------------------
        ...
        nb.fit(train_input_bow, np.ravel(train_labels))
        y_pred_class = nb.predict(test_input_bow)
        ...
        y_pred_prob_nb = nb.predict_proba(test_input_bow)[:, 1]
        ...
        mpl.show()
        labels.figure.savefig("Confusion Matrix Plot Directory\ Naive Bayes(BOW) Confusion Matrix for 8000 features")

#------------------------------------------------------------------------SVM--------------------------------------------------------------------------------------------------
        svc = SVC(kernel='linear', random_state=1)
        svc.fit(train_input_bow, np.ravel(train_labels))
        y_pred_svm = svc.predict(test_input_bow)
...
#
        ...
        grid_search_rf.fit(train_input_bow, np.ravel(train_labels))
        grid_search_rf.best_params_
        y_pred_rf = grid_search_rf.predict(test_input_bow)
        ...
        # Calculate predicted probabilities for test data
        y_pred_prob_rf = grid_search_rf.predict_proba(test_input_bow)[:, 1]
